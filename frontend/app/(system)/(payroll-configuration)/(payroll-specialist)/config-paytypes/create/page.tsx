...
export default function CreatePayType() {
  ...
  const [form, setForm] = useState<CreatePolicyPayload>({ //create policy payload
    PayType: "",
    amount: 0,
...
//done
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await CreatePayType(form);
      router.push('./');
    } catch (err: any) {
      setError(err?.response?.data?.message || err?.message || "Failed to create payType");
    } finally {
      setLoading(false);
    }
  };
