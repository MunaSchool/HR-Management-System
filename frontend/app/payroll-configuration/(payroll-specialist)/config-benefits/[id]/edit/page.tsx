      ...
      const fetchBenefit = async () => {
        try {
          const res = await axiosInstance.get(
            `/payroll-configuration/termination-resignation-benefits/${benefitId}`
          );
          setFormData(res.data);
          setError(null);
        } catch (err: any) {
          setError(err?.response?.data?.message || "Failed to fetch benefit");
        } finally {
          setLoading(false);
        }
      };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-50">Edit Benefit</h1>
          <p className="mt-1 text-gray-600 dark:text-gray-400">
            Update the termination or resignation benefit details
          </p>
        </div>
        <Link
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-gray-800 rounded-lg p-6 shadow">
        ...
        <div className="flex gap-4 pt-4">
          <button
            ...
            //onSubmit={() => }
          >
            {submitting ? "Saving..." : "Update Benefit"}
          </button>
          <Link
            href="../../config-benefits"
