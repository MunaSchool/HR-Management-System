...
export default function EditCyclePage() {
  ...
  const fetchCycle = async () => {
    try {
      ...
      setFormData({
        ...
        employeeAcknowledgementDueDate: data.employeeAcknowledgementDueDate ? new Date(data.employeeAcknowledgementDueDate) : undefined,
  ...
  const fetchDepartments = async () => {
    try {
      // Mock departments for now - you'll need to implement this
      setDepartments([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.employeeAcknowledgementDueDate) {
        updateData.employeeAcknowledgementDueDate = formData.employeeAcknowledgementDueDate.toISOString();
      }
      if (formData.status) {
      ...
      // Update the cycle
      // First, add the update method to your performanceApi
      ...
      <div className="flex justify-between items-start">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Link href={`/performance/cycles/view/${cycleId}`} className="text-gray-600 hover:text-gray-900">
              <ArrowLeft className="h-5 w-5" />
            </Link>
            <h1 className="text-2xl font-bold text-gray-900">Edit Cycle: {cycle?.name}</h1>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <input
                type="date"
                className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={formData.managerDueDate ? formData.managerDueDate.toISOString().split('T')[0] : ''}
            ...
            <div>
              ...
              <input
                type="date"
                className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={formData.employeeAcknowledgementDueDate ? formData.employeeAcknowledgementDueDate.toISOString().split('T')[0] : ''}
                ...
                <div key={index} className="p-4 border rounded-lg bg-white">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-gray-700">Assignment #{index + 1}</span>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                    ...
                    <div>
                      <label className="block text-xs text-gray-600 mb-1">Departments *</label>
                        ...
                        {departments.map((department) => (
                          <div key={department._id} className="flex items-center mb-1">
                            <input
                              ...
                              onChange={() => {
                                const newDeptIds = assignment.departmentIds.includes(department._id)
                                  ? assignment.departmentIds.filter(id => id !== department._id)
                                  : [...assignment.departmentIds, department._id];
                                
                                const newAssignments = [...(formData.templateAssignments || [])];
                                newAssignments[index] = { ...assignment, departmentIds: newDeptIds };
                                setFormData(prev => ({ ...prev, templateAssignments: newAssignments }));
                              }}
                              className="h-3 w-3 text-blue-600 rounded"
                            />
                            <label htmlFor={`assignment-${index}-dept-${department._id}`} className="ml-2 text-xs text-gray-700">
                              {department.name}
                            </label>
                          </div>
                        ))}
        ...
        <div className="bg-white border rounded-lg p-6 shadow-sm">
          <div className="flex justify-between">
            ...
            <div className="flex space-x-3">
              ...
              <button
                type="submit"
                disabled={saving || !formData.templateAssignments || formData.templateAssignments.length === 0}
