...
export default function ReportsPage() {
  ...
  const generateReport = async (reportId: string) => {
    try {
      // Show loading state
      const originalText = document.getElementById(`report-btn-${reportId}`)?.textContent;
      ...
      switch (reportId) {
        case 'performance-summary':
          reportData = await performanceApi.getPerformanceAnalytics(selectedCycle === 'all' ? undefined : selectedCycle);
          break;
        case 'cycle-analysis':
          ...
          break;
        default:
          reportData = await performanceApi.exportPerformanceReport(selectedCycle === 'all' ? undefined : selectedCycle);
      }
      ...
      setReportTypes(prev => prev.map(report => 
        report.id === reportId 
          ? { ...report, lastGenerated: new Date(), size: `${Math.round(blob.size / 1024)} KB` }
          : report
      ));
    ...
    <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 flex items-center justify-center gap-2"
              >
                <Download size={16} />
                Bulk Export
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportTypes.map((report) => (
          <div key={report.id} className="bg-white border rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="bg-white border rounded-lg shadow-sm">
        <div className="px-6 py-4 border-b">
          <h3 className="font-medium text-gray-900">Recent Report History</h3>
          <p className="text-sm text-gray-500 mt-1">Recently generated performance reports</p>
        </div>
        <div className="p-6">
                      ...
                      <tr key={report.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="px-3 py-1 text-xs font-medium bg-gray-100 text-gray-800 rounded-full">
                            {selectedCycle === 'all' ? 'All Cycles' : 
                              cycles.find(c => c._id === selectedCycle)?.name || 'Selected Cycle'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-3 py-1 text-xs font-medium rounded-full ${getFormatColor(report.format)}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-gray-50 rounded-lg border">
                <div className="flex items-center mb-3">
                  ...
                  <h5 className="font-medium">Schedule Reports</h5>
                </div>
                <p className="text-sm text-gray-600 mb-3">Automatically generate reports on a schedule</p>
              ...
              <div className="p-4 bg-gray-50 rounded-lg border">
                <div className="flex items-center mb-3">
                  ...
                  <h5 className="font-medium">Shared Reports</h5>
                </div>
                <p className="text-sm text-gray-600 mb-3">Reports shared with your team</p>
                <button className="text-sm text-blue-600 hover:text-blue-800 font-medium">
                  View shared →
                </button>
              </div>
              ...
              <div className="p-4 bg-gray-50 rounded-lg border">
                ...
                <p className="text-sm text-gray-600 mb-3">Create custom report templates</p>
                <button className="text-sm text-blue-600 hover:text-blue-800 font-medium">
                  Create custom →
                </button>
              </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-6">
        <div className="flex items-start">
          ...
          <div>
            <h4 className="font-medium text-blue-900 mb-1">Need help with reports?</h4>
            <p className="text-sm text-blue-700 mb-3">
              Our reports system supports JSON, CSV, PDF, and Excel formats. JSON is recommended for data analysis, 
              CSV for spreadsheets, PDF for presentations, and Excel for detailed analysis.
            </p>
